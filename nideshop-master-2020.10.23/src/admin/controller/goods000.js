    ...
    const product = this.model('product'),goods_specification = this.model('goods_specification'),goods_gallery = this.model('goods_gallery');
     let gallery = []
      ...
      if (id > 0) {
        ...
        if(gLen < len){          //可能原来只有3个图片，现在多上传了n个图片,此时这n个图片就变成新增了
        ...
        }else if(gLen > len){           //可能原来只有4个图片，现在去掉了2个图片,此时这2个图片就变成删除了        
          /*for (let j = 0; j < len; j++) { 
            gallery[j] = {}
            gallery[j].id = gallery_id[j];
            gallery[j].img_url = list_pic_gallery[j]
          ...
          for (let j = len; j < gLen-len; j++) { 
            ...
            gallery[j].id = gallery_id[j];
            gallery[j].goods_id = id   
            gallery[j].is_delete = 1  
          }

           await goods_gallery.updateMany(gallery);       ///先更新，再删除
           */
          /* console.log(11)
       ...
       /* let galleryArray = await goods_gallery.where({goods_id: id}).select();
        console.log(galleryArray);
        
        for (let j = 0; j < list_pic_gallery.length; j++) {   
          galleryArray[j].img_url = list_pic_gallery[j]
        }
        console.log(galleryArray);
        await goods_gallery.updateMany(galleryArray);
