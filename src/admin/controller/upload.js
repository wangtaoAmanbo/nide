...
module.exports = class extends Base {
  ...
  async brandPicAction() {
    ...
    const that = this;
    const filename = '/static/upload/brand/' + think.uuid(32) + utils.getImgType(imgFile);
 ...
 /* async brandNewPicAction() {
  ...
  async brandNewPicAction() {
    ...
    */
    /*is.on('end',function(){
      fs.unlinkSync(imgFile.path);    //no such file or directory, unlink 'C:\Users\ADMINI~1\AppData\Local\Temp\upload_4c47d4564115fc0cb20fb894bae5342a'
    });*/
  ...
  async goodsPicAction() {
    ...
    const filename = '/static/upload/goodsTemp/' + think.uuid(32) + utils.getImgType(imgFile);
    const is = fs.createReadStream(imgFile.path);
    const os = fs.createWriteStream(think.ROOT_PATH + '/www' + filename);
    is.pipe(os);
    ...
    const filePath = think.ROOT_PATH + '/www'+'/static/upload/goodsTemp/'+ymd+'/'
    ...
    return that.success({
      name: 'goodsPic',
      fileUrl: this.config('domain.name') + '/static/upload/goodsTemp/'+ymd+'/' + filename
    });*/
  }
 ...
 //首页轮播图片
  async galleryIndexAction() {
    ...
    const that = this;
    const filename = '/static/upload/index/' + think.uuid(32) + utils.getImgType(imgFile);
    const is = fs.createReadStream(imgFile.path);
    const os = fs.createWriteStream(think.ROOT_PATH + '/www' + filename);
    is.pipe(os);
  ...
  async imgIndexDeleteAction() {
   ...
   /* const utils =this.service('utils', 'admin');
    await utils.deleteall(filePath)
    return this.success({
      result: filePath+' is delete success'
 ...
 //删除商品轮播图片
  ...
  async topicDetailPicAction() {
    ...
    const utils =this.service('utils', 'admin');
    const that = this;
    const filename = '/static/upload/topic/' + think.uuid(32) + utils.getImgType(imgFile);
    const is = fs.createReadStream(imgFile.path);
    const os = fs.createWriteStream(think.ROOT_PATH + '/www' + filename);
    is.pipe(os);

    return that.success({
      name: 'topic_detail_pic',
      fileUrl: this.config('domain.name') + filename
    });
  }
  ...
  async categoryWapBannerPicAction() {
    ...
    const utils =this.service('utils', 'admin');
    const that = this;
    const filename = '/static/upload/category/' + think.uuid(32) + utils.getImgType(imgFile);
