...
module.exports = class extends Base {
  ...
  async addAction() {
    ...
    const cartInfo = await this.model('cart').where({goods_id: goodsId, product_id: productId,user_id:this.getLoginUserId()}).find();
    ...
    if (think.isEmpty(cartInfo)) {
      ...
      //model.thenAdd(data, where) ,当 where 条件未命中到任何数据时才添加数据。
  ...
  async updateAction() {
    ...
    if (think.isEmpty(newCartInfo)) {
      ...
      /*if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSepcifition = await this.model('goods_specification').field(['mobieshop_goods_specification.*', 'mobieshop_specification.name']).join('mobieshop_specification ON mobieshop_specification.id=mobieshop_goods_specification.specification_id').where({
          'mobieshop_goods_specification.goods_id': goodsId,
          'mobieshop_goods_specification.id': {'in': productInfo.goods_specification_ids.split('_')}
        }).select();
      }*/
  ...
  async checkoutAction() {
   ...
   /* if (addressId) {
    ...
    const freight = this.service('freight', 'api');
    console.log(freight.getFreight(addressId,userId));

    const fr = await this.service('freight', 'api').getFreight(addressId,userId);
    console.log(fr);*/
