...
module.exports = class extends Base {
  ...
  async detailAction() {
    ...
    const gallery = await this.model('goods_gallery').where({goods_id: goodsId,enabled :1}).limit(5).select();
    //const attribute = await this.model('goods_attribute').field('mobieshop_goods_attribute.value, mobieshop_attribute.name').join('mobieshop_attribute ON mobieshop_goods_attribute.attribute_id=mobieshop_attribute.id').order({'mobieshop_goods_attribute.id': 'asc'}).where({'mobieshop_goods_attribute.goods_id': goodsId}).select();
    const attribute = await this.model('goods_attribute').field('mobieshop_goods_attribute.value, mobieshop_attribute.name').join('mobieshop_attribute ON mobieshop_goods_attribute.attribute_id=mobieshop_attribute.id').order({'mobieshop_goods_attribute.id': 'asc'}).where({'mobieshop_goods_attribute.goods_id': goodsId}).select();
    ...
    if (!think.isEmpty(hotComment)) {
      ...
      let c= Buffer.from(hotComment.content, 'base64').toString();
      //let e = c.replace(/网易/g,'').replace(/严选/g,'运达');
      let e = c.replace(/网易/g,'').replace(/严选/g,'amanbo');
      commentInfo = {
